...
def trKsquare(B, A):
    # inputs: A, B: two tensors of size 2 x 2 x N x N
    # output kappa = tr(K^2) of size ..., where K = logm(inv(B)A)
    ...
    lambda1 = w1 + w3 * tau
    lambda2 = w2 - w3 * tau
    return torch.log(lambda1) ** 2 + torch.log(lambda2) ** 2


def get_inverse(A):  # compute the inverse of the first 2D slice of a 2 x 2 x N x N matrix

    detA = A[0, 0] * A[1, 1] - A[0, 1] * A[1, 0] + 1e-40

    C = torch.zeros(A.size(), dtype=torch.double)
    C[0, 0] = A[1, 1]
    C[1, 1] = A[0, 0]
    C[0, 1] = -A[0, 1]
    C[1, 0] = -A[1, 0]
    return C / detA  # C: 2 x 2 x N x N
...
def Squared_distance_Ebin(g0, g1, a, mask):  # g0,g1: two tensors of size 2 x 2 x N x N
    # g0 = torch.einsum("mnij,kij->mnij", [g0, mask])
    # g1 = torch.einsum("mnij,kij->mnij", [g1, mask])
    # g0[0, 0, :, :][mask.squeeze() == 0] = 1
    # g0[1, 1, :, :][mask.squeeze() == 0] = 1
    # g1[0, 0, :, :][mask.squeeze() == 0] = 1
    # g1[1, 1, :, :][mask.squeeze() == 0] = 1
    inv_g0 = get_inverse(g0)
    #     matrix multiplication
    inv_g0_g1 = torch.einsum("ik...,kj...->...ij", [inv_g0, g1])
    trK0square = trKsquare(g0, g1) - torch.log(
        get_det(inv_g0_g1) + 1e-40) ** 2 / 2  # check the order of the function trKsquare
    ...
    det_g0 = g0[0, 0] * g0[1, 1] - g0[0, 1] * g0[1, 0] + 1e-40
    det_g1 = g1[0, 0] * g1[1, 1] - g1[0, 1] * g1[1, 0] + 1e-40

    alpha, beta = det_g0.pow(1 / 4), det_g1.pow(1 / 4)
    #     SplitEbinMetric Eq.(3)
    E = 16 * a * (alpha ** 2 - 2 * alpha * beta * torch.cos(theta) + beta ** 2)
    return torch.sum(E)
    # return torch.einsum("...ij,kij->", [E, mask])
...
def logm_invB_A(B, A):
    ...
    #     matrix multiplication
    return torch.einsum("ik...,ks...,sj...->...ij", [V, Lambda, get_inverse(V)])
...
def inv_RieExp(g0, g1, a):  # g0,g1: two tensors of size (s,t,...,2,2), where g0\neq 0
    ...
    def get_u_ng0direction(g0, g1, inv_g0_g1, a):  # (-1,2,2) first reshape g0,g1,inv_g..
        ...
        det_g0 = g0[:, 0, 0] * g0[:, 1, 1] - g0[:, 0, 1] * g0[:, 1, 0]
        det_g1 = g1[:, 0, 0] * g1[:, 1, 1] - g1[:, 0, 1] * g1[:, 1, 0]
        gamma = det_g1.pow(1 / 4) / (det_g0.pow(1 / 4))  # (-1)
...
def Rie_Exp_extended(g0, u, a):  # here g0 is of size (s,t,...,2,2) and u is of size (s,t,...,2,2)
    ...
    # detg0 = torch.det(g0)
    detg0 = g0[:, 0, 0] * g0[:, 1, 1] - g0[:, 0, 1] * g0[:, 1, 0]
...
def ptPick_notInRange(g0, g1, i):  # (-1,2,2)
    det_g0 = g0[:, 0, 0] * g0[:, 1, 1] - g0[:, 0, 1] * g0[:, 1, 0]
    det_g1 = g1[:, 0, 0] * g1[:, 1, 1] - g1[:, 0, 1] * g1[:, 1, 0]
    alpha = det_g1.pow(1 / 4) / det_g0.pow(1 / 4)  # (-1)
...
if __name__ == "__main__":
    Num = 5
    m, n = 7, 8
    G = torch.randn(Num, m, n, 2, 2, dtype=torch.double)  # remove m or/and n to test the other cases of size (-1,2,2) and (2,2)
    G = torch.einsum("...ik,...jk->...ij", [G, G])
    # G[1] = 0
    a = 1 / 2
    gm = get_KarcherMean(G, a)
    em, vm = torch.symeig(gm, eigenvectors=True)
    fig = plt.figure(figsize=(5, 5))
    plot_PDSMs(em / abs(em.max()), vm)
    plot_geo(G, G.size(0))
